...
def maybe_get_set_global_memory_buffer(q, k, v, m, l, o, bias_args):
    ...
    # if _PEER_Q is None:
    if _PEER_Q is None or q.shape != _PEER_Q[0].shape:
        # try:
...
def maybe_get_set_global_memory_buffer_bwd(dq, dk, dv, q, L, k, v, o, do, bias_args):
    ...
    if _DELTA_DQ is None or dq.shape != _DELTA_DQ[0].shape:
        # try:
        #     if get_sequence_parallel_rank() == 0:
...
def reset_global_memory_buffer():
    ...
    global _PEER_Q_BWD, _PEER_O_BWD
    _PEER_Q = None
    _PEER_K = None
    _PEER_V = None
    _PEER_M = None
...
def maybe_send_recv_fwd_qkvo(q: torch.Tensor, peer_q: torch.Tensor,
    ...
    if False:
        ...
        all_handles.append(P2POp(op=isend, tensor=k, peer=maybe_send_rank % seq_world_size, group=seq_group))
        all_handles.append(P2POp(op=isend, tensor=v, peer=maybe_send_rank % seq_world_size, group=seq_group))
        if debug:
    ...
    if False:
        ...
        all_handles.append(P2POp(op=irecv, tensor=peer_k, peer=maybe_recv_rank % seq_world_size, group=seq_group))
        all_handles.append(P2POp(op=irecv, tensor=peer_v, peer=maybe_recv_rank % seq_world_size, group=seq_group))
        if debug:
...
def send_recv_fwd_q(q_send, q_recv, bias_args_send, bias_args_recv, comm_mode):
    # bias_args_send, bias_args_recv: tuple of tensors
    seq_group = get_sequence_parallel_group()
    seq_rank = get_sequence_parallel_rank()
    seq_world_size = get_sequence_parallel_size()
    ...
    all_handles.append(P2POp(op=isend, tensor=q_send, peer=send_rank, group=seq_group))
    all_handles.append(P2POp(op=irecv, tensor=q_recv, peer=recv_rank, group=seq_group))
...
def send_recv_fwd_mlo(m_send, m_recv, l_send, l_recv, o_send, o_recv, comm_mode):
    seq_group = get_sequence_parallel_group()
    seq_rank = get_sequence_parallel_rank()
    seq_world_size = get_sequence_parallel_size()
    ...
    all_handles.append(P2POp(op=isend, tensor=m_send, peer=send_rank, group=seq_group))
    all_handles.append(P2POp(op=isend, tensor=l_send, peer=send_rank, group=seq_group))
    all_handles.append(P2POp(op=isend, tensor=o_send, peer=send_rank, group=seq_group))
    
    all_handles.append(P2POp(op=irecv, tensor=m_recv, peer=recv_rank, group=seq_group))
    all_handles.append(P2POp(op=irecv, tensor=l_recv, peer=recv_rank, group=seq_group))
    all_handles.append(P2POp(op=irecv, tensor=o_recv, peer=recv_rank, group=seq_group))

    all_reqs = launch_async_handles(all_handles, comm_mode)
    return [all_reqs]

def send_recv_fwd_Lo(L_send, L_recv, o_send, o_recv, comm_mode):
    seq_group = get_sequence_parallel_group()
    seq_rank = get_sequence_parallel_rank()
    seq_world_size = get_sequence_parallel_size()
...
def send_recv_bwd_comp_reqs(q_send, L_send, o_send, do_send, bias_args_send,
                             q_recv, L_recv, o_recv, do_recv, bias_args_recv, comm_mode):
    seq_group = get_sequence_parallel_group()
    seq_rank = get_sequence_parallel_rank()
    seq_world_size = get_sequence_parallel_size()
    ...
    send_rank = (seq_rank + 1) % seq_world_size
    recv_rank = (seq_rank - 1) % seq_world_size
    # print(f"R={seq_rank} send q, L, o, do to {send_rank} recv from {recv_rank}")

    all_handles.append(P2POp(op=isend, tensor=q_send, peer=send_rank, group=seq_group))
    all_handles.append(P2POp(op=isend, tensor=L_send, peer=send_rank, group=seq_group))
    all_handles.append(P2POp(op=isend, tensor=o_send, peer=send_rank, group=seq_group))
    all_handles.append(P2POp(op=isend, tensor=do_send, peer=send_rank, group=seq_group))
    for i in range(len(bias_args_send)):
        all_handles.append(P2POp(op=isend, tensor=bias_args_send[i], peer=send_rank, group=seq_group))
    ...
    all_handles.append(P2POp(op=irecv, tensor=L_recv, peer=recv_rank, group=seq_group))
    all_handles.append(P2POp(op=irecv, tensor=o_recv, peer=recv_rank, group=seq_group))
    all_handles.append(P2POp(op=irecv, tensor=do_recv, peer=recv_rank, group=seq_group))
...
def send_recv_bwd_rescale_reqs(dq_delta_send, dq_delta_recv, comm_mode):
    seq_group = get_sequence_parallel_group()
    seq_rank = get_sequence_parallel_rank()
    seq_world_size = get_sequence_parallel_size()
    ...
    send_rank = (seq_rank + 1) % seq_world_size
    recv_rank = (seq_rank - 1) % seq_world_size
    # print(f"R={seq_rank} send dq_delta to {send_rank} recv from {recv_rank}")

    all_handles.append(P2POp(op=isend, tensor=dq_delta_send, peer=send_rank, group=seq_group))
    all_handles.append(P2POp(op=irecv, tensor=dq_delta_recv, peer=recv_rank, group=seq_group))
